...
Talks to the EIS socket gamescope opens at `$XDG_RUNTIME_DIR/gamescope-<N>-ei`
...
== Empirical status on this stack ==

`move()` works end-to-end (verified against `us xeyes`: pupils tracked,
gamescope's seat cursor reported the expected position).

`click()` does NOT dismiss Timberborn's mod-load dialog on this stack,
while xdotool clicks at the same coordinate do (phash diff 16 between
the two outcomes). Root cause unknown — three plausible hypotheses:

  (a) bSynthetic=true is set by gamescope's libei handler
      (InputEmulation.cpp:184); if Unity gates click processing on a
      cursor-hidden-state update that bSynthetic suppresses, the click
      gets filtered.
  (b) wlserver_mousewarp teleports cursor coords without triggering
      steamcompmgr's focus-update path that wlserver_mousemotion does
      (steamcompmgr.cpp:4365), so the button lands on whatever
      mouse_focus_surface happened to be set.
  (c) XWayland dispatch timing: motion → XWayland-pointer-state →
      X11-dispatch has more hops than xdotool's direct XTest path,
      so a click can dispatch to the wrong X11 window if motion hasn't
      propagated yet.

Note that libei and physical input events converge at the same wlroots
seat (wlserver_mousebutton → wlr_seat_pointer_notify_button); there's no
separate "gamescope libei seat". Whatever is breaking click delivery is
downstream of that join point.

Diagnostic recipe: `WAYLAND_DEBUG=1` on the game process during a libei
click. If the button reaches the game surface, the bug is in Proton's
Wayland input handling; if it doesn't, the bug is in gamescope's wlroots
dispatch.
"""
