...
3. **libei** (opt-in via `--backend libei`): events go directly to gamescope's
   ...
   positioning (`move()`) works end-to-end on this stack. Click (`button`)
   delivery does NOT dismiss Timberborn UI dialogs while xdotool clicks at
   the same coord do (phash diff 16). Root cause unknown — libei and physical
   input converge at the same wlroots seat (`wlserver_mousebutton`), so the
   break is downstream of that join. See `libei_backend.py` docstring for
   the three open hypotheses (bSynthetic suppression, stale
   `mouse_focus_surface`, XWayland dispatch timing) and the diagnostic
   recipe. Kept opt-in because move() is useful and the protocol integration
   is correct.
